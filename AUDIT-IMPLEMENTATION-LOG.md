...
|---|---|---|---|
| `index.html` | Preserve visitor intent, improve conversion clarity, and remove redundant hero copy. | Routed project and review CTAs separately; standardized **Free Initial Review**; replaced the text-heavy priority interface with a decorative floating orbital mark; removed the outer route diagram and visible capability Pause button; expanded the capability track to four synchronized copies while exposing only the first to assistive technology; kept the intentional three-line hero and two-line contact headings; retained the original form endpoint and integrations. | Static metadata/link/form/schema audit; project/review routing checks; orbit and ticker measurements; no-text/no-control assertions; loop-boundary and full-cycle observation; responsive browser matrices. |
...
| File | Why it changed | What changed | How it was validated |
|---|---|---|---|
| `assets/css/site.css` | Prevent awkward heading wraps and implement the approved text-free hero treatment. | Added explicit heading-line blocks and responsive safeguards; removed obsolete priority-panel, dark-card, grid, background, route, and corner-node styles; added a transparent code-native orbital treatment with theme-aware rings and satellites; removed doubled ticker-copy padding; changed the ticker translation to exactly one of four equal copies; retained explicit responsive and reduced-motion states. | Contact heading measured at 1440, 390, and 320 px; orbit/satellite/core motion observation; light/dark visual inspection; transparent background/border/shadow assertions at seven widths; 0 px ticker seams through 2560 px; 100-case overflow/breakpoint matrix. |
| `assets/js/site.js` | Preserve CTA intent, make native validation errors perceivable, and remove dead interface code. | Added query-based project/review selection; capture-phase invalid handling; `aria-invalid` state; polite live announcement; clearing/recovery behavior; removed obsolete hero-tab and marquee-button handlers; retained native Formspree submission and remaining interactions. | Node syntax check; project/review/default checks; empty and corrected-form checks without submission; source search for removed handlers; browser console 0 errors/warnings. |
...
| `tools/site_audit.py` | Make important release checks repeatable. | Expanded standard-library checks for links/fragments, metadata, sitemap parity/dates, redirects, headings, JSON-LD, forms, images, alt classification, orphan pages, representative disclosures, placeholders, legal dates, pricing/currency/cadence, exact integrations, unsupported numerical claims, and deployment artifacts. P0/P1 technical regressions exit nonzero. | Final bundled-Python run: 30 HTML files, 25 indexable pages, 242 references, 59 images, 35 JSON-LD blocks, 2 forms, 4 redirects, 0 warnings, 0 critical failures. |
...
| Asset references checked | 242 |
| Images checked for dimensions and alt behavior | 59 |

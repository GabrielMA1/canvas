...
|---|---|---|---|
| `index.html` | Preserve visitor intent, improve conversion clarity, and remove redundant hero copy. | Routed project and review CTAs separately; standardized **Free Initial Review**; replaced the text-heavy priority interface with a decorative connected-signal animation; removed the visible capability Pause button while adding a focus/hover instruction; kept the intentional three-line hero and two-line contact headings; retained the original form endpoint and integrations. | Static metadata/link/form/schema audit; project/review routing checks; animation and ticker-motion measurements; no-text/no-control assertions; final 10-viewport homepage pass and 100-case browser matrix. |
...
| File | Why it changed | What changed | How it was validated |
|---|---|---|---|
| `assets/css/site.css` | Prevent awkward heading wraps and implement the approved text-free hero treatment. | Added explicit heading-line blocks and responsive safeguards; removed obsolete priority-panel styles; added a code-native connected-signal animation, restrained edge fades, focus/hover ticker pausing, and explicit responsive and reduced-motion states. | Contact heading measured at 1440, 390, and 320 px; animation-state observation; light/dark inspection; final 10-viewport homepage pass and 100-case overflow/breakpoint matrix. |
